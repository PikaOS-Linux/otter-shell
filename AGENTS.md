...
- **Bar widget specs** (otter-ui): All 17 bar widgets emit frames via `otter-ui/src/widgets/specs/*.zig`; widget files own state and `drawFrame` emitters. `FieldRegistry` calls struct `getWidth`/`setArea`/`deinit` directly (child `Widget` shells hold area/motion only).
...
- **Text file application matching** (otter-desktop): Native XDG application enumeration and explicit launch share MIME matching. `text/plain` editors accept `text/*` source files, including Lua and C; binary types remain excluded.
